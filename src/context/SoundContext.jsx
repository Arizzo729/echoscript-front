...
export function SoundProvider({ children, initialVolume = 0.4 }) {
  ...
  useEffect(() => {
    try {
      ...
      const savedVol = saved.volume ?? initialVolume;
      setVolume(savedVol);
      setTrackIndex(saved.trackIndex ?? OFF_INDEX);
      
      if (mainAudioRef.current) {
        mainAudioRef.current.muted = saved.isMuted ?? true;
        mainAudioRef.current.volume = savedVol * (TRACKS[saved.trackIndex ?? OFF_INDEX].gain || 0);
      }
    } catch (e) {
  ...
  const togglePlay = useCallback(() => {
    ...
    if (audio.paused) {
      audio.play().then(() => {
        fadeTo(audio, volume * track.gain);
        setIsPlaying(true);
        setIsMuted(false); // Ensure unmuted when playing
