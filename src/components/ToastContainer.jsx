...
// Position presets
type PositionKey = "top-right" | "top-left" | "bottom-right" | "bottom-left";
...
export default function ToastContainer({ position = "top-right" }: { position?: PositionKey }) {
  ...
  return createPortal(
    <div className={`${POSITIONS[position]} z-50 space-y-3 p-2 max-w-sm pointer-events-none`}>
      <AnimatePresence>
        {toasts.map((t) => {
          const v = VARIANTS[t.type as ToastType] || VARIANTS.info;
          return (
            <motion.div
              key={t.id}
