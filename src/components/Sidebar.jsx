...
export default function Sidebar() {
  ...
  return (
    <motion.aside
      ...
      className={`fixed top-0 left-0 h-screen z-40 bg-gradient-to-b from-zinc-950 via-zinc-900 to-zinc-950 border-r border-zinc-800 shadow-2xl backdrop-blur-lg transition-all duration-300 hidden md:block ${collapsed ? "w-16" : "w-56"}`}
    >
      ...
      <nav className="mt-6 flex flex-col gap-1 px-2">
        {navItems.map(({ path, label, icon: Icon }) => (
          ...
          >
            ...
            <AnimatePresence>
              {!collapsed && (
                <motion.span initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -5 }} className="truncate">
                  {label}
                </motion.span>
              )}
            </AnimatePresence>
