...
function useTouchDrag({ enabled = true, onDragEnd, initial = { x: 0, y: 0 } }) {
  ...
  useEffect(() => {
    ...
    function onTouchMove(e) {
      ...
      y = Math.max(0, Math.min(window.innerHeight - height - 12, y));
      setPos({ x, y });
    }
...
export default function AudioOverlay() {
  ...
  const MobileOverlay = (
    ...
    >
      ...
      <input
        ...
        className="w-16 h-1 accent-teal-400 cursor-pointer mx-1"
        aria-label="Volume"
        style={{ minWidth: 48 }}
      />
      {/* Collapse */}
  ...
  // --- DESKTOP OVERLAY (unchanged, still draggable) ---
