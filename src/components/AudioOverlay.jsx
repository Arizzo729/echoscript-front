...
// --- DRAG LOGIC for mobile (no lag, sticky under finger) ---
function useTouchDrag({ enabled = true, onDragEnd, initial = { x: 0, y: 0 } }) {
  ...
  useEffect(() => {
    ...
    function onTouchMove(e) {
      ...
      // Clamp so overlay never goes off screen or over nav (up 100px)
      const card = node;
      const width = card.offsetWidth;
      const height = card.offsetHeight;
      x = Math.max(0, Math.min(window.innerWidth - width, x));
      y = Math.max(0, Math.min(window.innerHeight - height - 80, y)); // 80px from bottom to avoid nav
      setPos({ x, y });
    }
...
export default function AudioOverlay() {
  ...
  // --- Fix: Always switch icon on click by using state directly from useSound ---
  ...
  const MobileFAB = (
    <motion.button
      ...
      style={{
        position: 'fixed',
        bottom: 82, // Move up to clear nav
        right: 18,
        zIndex: 9999,
  ...
  // --- DESKTOP OVERLAY (unchanged design, fits everything inside, collapse button inside bar) ---
