    ...
    <>
      ...
      <AnimatePresence>
        <motion.div
          ...
          className="hidden md:flex fixed z-[9999] flex-col items-center gap-1 select-none shadow-lg"
        >
          {showTip && (
      ...
      <AnimatePresence>
        {mobileVisible && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 40 }}
            transition={{ type: 'spring', stiffness: 200, damping: 20 }}
            className="flex md:hidden fixed bottom-6 left-1/2 transform -translate-x-1/2 items-center gap-2 px-2 py-1 bg-zinc-800/90 backdrop-blur border border-teal-400/50 rounded-full shadow-lg z-[9999]"
          >
            <Button
              variant="ghost"
              size="sm"
            ...
            >
              <X className="w-4 h-4 text-teal-300" />
            </button>
          </motion.div>
        )}
      </AnimatePresence>
