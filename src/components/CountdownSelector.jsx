...
export default function CountdownSelector({ value, onChange, options = [0, 1, 2, 3, 5, 7, 10] }) {
  ...
  return (
    ...
    >
      ...
      >
        ⏱ {t("upload.countdown_label", "Countdown Before Recording")}
      </label>
        ...
        >
          {options.map((option) => (
            <option key={option} value={option}>
              {option === 0 ? t("upload.no_countdown", "No Countdown") : `${option} ${t("upload.seconds", "seconds")}`}
            </option>
          ))}
        </select>
      ...
      <p className="mt-1 text-xs text-zinc-500">
        {t("upload.countdown_help", "Choose how long you want to prepare before recording starts.")}
      </p>
    </motion.div>
  );
}
