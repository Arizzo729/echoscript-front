...
export default function OnboardingModal({ onClose }) {
  ...
  // When video finishes or skipped
  const handleVideoFinish = useCallback(() => setShowVideo(false), []);

  // Render the intro video first
  if (showVideo) {
    return (
      <IntroVideo
        sources={[
  ...
  const swipeHandlers = useSwipeable({ onSwipedLeft: () => setStep(s => Math.min(s + 1, STEPS.length - 1)), onSwipedRight: () => setStep(s => Math.max(s - 1, 0)), trackMouse: true });
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          <button onClick={finishOnboarding} className="absolute top-2.5 right-2.5 text-teal-400 p-1">
            <X className="w-5 h-5" />
          </button>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={step} className="px-6 py-5 flex flex-col items-center text-center"
              initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -100, opacity: 0 }} transition={{ duration: 0.35 }}
            >
              <div className="w-full h-52 bg-zinc-800 rounded-xl flex items-center justify-center mb-4 shadow-inner">
                {animData ? <Lottie animationData={animData} loop autoplay style={{ width: '90%', height: '100%' }} /> : <div className="animate-spin h-8 w-8 border-4 border-teal-400 border-t-transparent rounded-full" />}
              </div>
              <h2 id="onboarding-title" className="text-2xl font-semibold mb-2 tracking-tight">{current.title}</h2>
              <p id="onboarding-desc" className="text-sm text-zinc-400 leading-relaxed min-h-[3rem]">{typedDesc}</p>
            </motion.div>
          </AnimatePresence>

          <div className="flex justify-between items-center px-6 pb-4">
            <button onClick={() => setStep(s => Math.max(s - 1, 0))} disabled={step === 0} className="text-sm text-zinc-300 disabled:opacity-40">← Back</button>
            {step < STEPS.length - 1 && <button onClick={() => setStep(s => Math.min(s + 1, STEPS.length - 1))} className="text-sm text-teal-300">Next →</button>}
          </div>

          {current.id === "audio" && <div className="px-6 pb-4 text-center"><button onClick={() => { enableSound(); setStep(s => s + 1); }} className="px-5 py-2 rounded-full bg-teal-500">Enable Audio</button></div>}
          {current.id === "start" && <div className="px-6 pb-5 flex flex-col items-center space-y-2"><button onClick={finishOnboarding} className="px-5 py-2 bg-teal-500 w-full">Finish</button><button onClick={finishOnboarding} className="text-xs text-teal-300">Do not ask again</button></div>}
        </motion.div>
      </motion.div>
    </AnimatePresence>
  );
