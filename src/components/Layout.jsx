...
export default function Layout() {
  ...
  // Global searchable pages used by header/mobile search
  const searchablePages = [
    { label: 'Dashboard', path: '/' },
    { label: 'Transcripts', path: '/transcripts' },
    { label: 'Upload', path: '/upload' },
    { label: 'Video Upload', path: '/video' },
    { label: 'Audio Upload', path: '/upload' },
    { label: 'Summarize', path: '/summary' },
    { label: 'Summary', path: '/summary' },
    { label: 'Usage Summary', path: '/purchase' },
  ...
  // Handle global search from header/mobile. If a page matches the query navigate there,
  // otherwise route to the transcripts page with a search query parameter.
  const handleGlobalSearch = (query) => {
    const q = (query || '').trim();
    if (!q) return;
    const match = searchablePages.find((p) => p.label.toLowerCase().includes(q.toLowerCase()));
    if (match) {
      navigate(match.path);
    } else {
      navigate(`/transcripts?search=${encodeURIComponent(q)}`);
    }
  };
      ...
      <ThemeContext.Provider value={themeValue}>
        ...
        {!showIntro &&
          (isMobile ? (
            ...
            <MobileLayout onSearch={handleGlobalSearch}>
              <ErrorBoundary>
                <Outlet />
              </ErrorBoundary>
            </MobileLayout>
            ...
            <div className="flex flex-col h-screen w-screen overflow-hidden bg-gradient-to-br from-[#0a0f1f] via-[#040711] to-[#050a15] text-white">
              <Suspense fallback={<div className="h-16 w-full bg-zinc-900" />}>
                <Header
                  ...
                  onSearch={(q) => handleGlobalSearch(q)}
                />
              </Suspense>
