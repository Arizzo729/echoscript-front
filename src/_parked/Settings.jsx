...
export default function Settings() {
  ...
  return (
    ...
    >
      ...
      <div className="flex space-x-4 mb-6">        {tabs.map((tab) => (
          <button
            key={tab.id}
...
function ToggleRow({ label, value, onChange, icon }) {
  return (
    <div className="flex items-center justify-between bg-zinc-800 px-4 py-3 rounded-lg border border-zinc-700 transition-all duration-300">
      <div className="flex items-center gap-3 text-sm text-zinc-300">
        {icon && <div className="text-teal-400">{icon}</div>}        <span>{label}</span>
      </div>
      <Switch
        checked={value}
        onChange={onChange}
...
function SliderRow({ label, value, onChange, min, max, step, display }) {
  return (
    <div className="flex items-center gap-4">
      <p className="text-sm text-zinc-400">{label}</p>      <input
        type="range"
        value={value}
