...
export default function Account() {
  ...
  // Sync local user state with auth context when available
  useEffect(() => {
    if (!authUser) return;
    setUser((prev) => ({
      ...prev,
      ...
      minutesUsed: authUser.minutesUsed ?? prev.minutesUsed,
      sessions: authUser.sessions ?? prev.sessions,
      avatar: authUser.avatar || prev.avatar,
      isGuest: (authUser.plan || "").toLowerCase() === "guest",
    }));
  }, [authUser]);
