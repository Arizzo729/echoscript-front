...
export default function Contact() {
  ...
  useEffect(() => {
    try {
      // expose which backend the client will call
      const maybeEnv = (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.VITE_API_BASE) ? import.meta.env.VITE_API_BASE : null;
      console.log('Frontend API base:', window.__VITE_API_BASE || maybeEnv || 'not-set');
    } catch (e) {}
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // api.call attaches parsed body as `body` on the thrown error when available
      const body = err && (err.body || err.response || null);
      let msg = err?.message || "Failed to send contact";
      try {
        if (body) {
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="grid gap-4">
        {/* Honeypot field for bots (backend will ignore if empty) */}
        <input type="text" name="hp" value={formData.hp || ""} onChange={handleChange} className="hidden" autoComplete="off" />
        <input
          id="name"
          type="text"
          name="name"
        ...
        {status === "error" && (
          <div className="mt-2">
            ...
            {errorMessage ? (
              <pre className="mt-2 text-xs text-zinc-300 bg-zinc-800 p-2 rounded">{errorMessage}</pre>
            ) : null}
          </div>
        )}
      </form>
