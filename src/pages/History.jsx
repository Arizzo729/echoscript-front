...
export default function HistoryPage() {
  ...
  const handleDelete = (id) => {
    ...
    if (itemToDelete) {
      ...
      localStorage.setItem("deletedHistory", JSON.stringify([itemToDelete, ...deleted]));
    }
    updateHistory(history.filter((h) => h.id !== id));
    toast({ message: t("Entry moved to recovery"), type: "info" });
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <h1 className="text-3xl font-bold" data-i18n="history.title">{t("history.title", "Your History")}</h1>
      ...
      {history.length === 0 ? (
        <p className="text-zinc-400 text-sm">{t("history.no_past_sessions_found", "No past sessions found.")}</p>
      ) : (
      ...
      <div className="mt-8 flex justify-center">
        ...
        >
          ...
          {t("history.recover_deleted_files", "Recover Deleted Files")}
        </button>
      </div>
    </motion.div>
  );
