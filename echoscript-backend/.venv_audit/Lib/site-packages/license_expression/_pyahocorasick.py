...
class Trie(object):
        ...
        for token_string in tokens:
            ...
            while match is not nil:
                if match.output is not nil:
                    ...
                    if TRACE:
                        logger_debug(
                            " type output", repr(output_value), type(matched_string)
                        )
                    ...
                    yield Token(
                        start_pos,
                        end_pos,
                        tokens_string[start_pos : end_pos + 1],
                        output_value,
                    )
                    yielded = True
                match = match.fail
            if not yielded and include_unmatched:
                ...
                start_pos = end_pos - n + 1
                tok = Token(
                    start_pos, end_pos, tokens_string[start_pos : end_pos + 1], None
                )
                if TRACE:
    ...
    def tokenize(self, string, include_unmatched=True, include_space=False):
        ...
        """
        tokens = self.iter(
            string, include_unmatched=include_unmatched, include_space=include_space
        )
        tokens = list(tokens)
        if TRACE:
...
class Token(object):
    ...
    def __repr__(self):
        return (
            ...
            + "(%(start)r, %(end)r, %(string)r, %(value)r)" % self.as_dict()
        )
