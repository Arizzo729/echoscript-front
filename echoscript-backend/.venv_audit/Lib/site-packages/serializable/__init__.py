    ...
    def default(self, o: Any) -> Any:
        ...
        if isinstance(o, object):
            ...
            serializable_property_info = (
                ObjectMetadataLibrary.klass_property_mappings.get(
            ...
            for k, prop_info in serializable_property_info.items():
                ...
                if not _allow_property_for_view(
                    prop_info=prop_info, view_=self._view, value_=v
                ):
                    # Skip as rendering for a view and this Property is not registered form this View
                    continue

                new_key = BaseNameFormatter.decode_handle_python_builtins_and_keywords(
                    name=k
                )
                ...
                if prop_info.custom_type:
                    ...
                    if isinstance(v, Decimal):
                        ...
                        if (
                            global_klass_name
                            not in ObjectMetadataLibrary.klass_mappings
                        ):
                            if prop_info.string_format:
...
class _JsonSerializable(Protocol):
    ...
    def from_json(cls: Type[_T], data: Dict[str, Any]) -> Optional[_T]:
        ...
        if klass is None:
            _logger.warning(
                "%s is not a known serializable class",
                klass_qualified_name,
                stacklevel=2,
            )
            return None
        ...
        for k, v in data.items():
            ...
            if decoded_k not in klass_properties:
                ...
                for p, pi in klass_properties.items():
                    if (
                        pi.custom_names.get(SerializationType.JSON)
                        in _allowed_custom_names
                    ):
                        new_key = p
            else:
                new_key = decoded_k
...
class _XmlSerializable(Protocol):
    ...
    def as_xml(
        ...
        for k, v in self.__dict__.items():
            ...
            if new_key in serializable_property_info:
                ...
                if not _allow_property_for_view(
                    prop_info=prop_info, view_=view_, value_=v
                ):
                    # Skip as rendering for a view and this Property is not registered form this View
                    continue
                ...
                if prop_info and prop_info.is_xml_attribute:
                    ...
                    if CurrentFormatter.formatter:
                        new_key = CurrentFormatter.formatter.encode(
        ...
        for k, prop_info in serializable_property_info.items():
            ...
            if not prop_info:
                raise ValueError(
                    f"{new_key} is not a known Property for {klass_qualified_name}"
                )
            ...
            if not prop_info.is_xml_attribute:
                ...
                if v is None:
                    SubElement(
                        this_e, _namespace_element_name(tag_name=new_key, xmlns=xmlns)
                    )
                    continue

                if new_key == ".":
                    this_e.text = _xs_string_mod_apply(
                ...
                if prop_info.is_array and prop_info.xml_array_config:
                    ...
                    for j in v:
                        if not prop_info.is_primitive_type() and not prop_info.is_enum:
                            ...
                            SubElement(nested_e, nested_key).text = (
                                _xs_string_mod_apply(
    ...
    def from_xml(
        ...
        klass = ObjectMetadataLibrary.klass_mappings.get(
            f"{cls.__module__}.{cls.__qualname__}"
        )
        if klass is None:
            _logger.warning(
                "%s.%s is not a known serializable class",
                cls.__module__,
                cls.__qualname__,
                stacklevel=2,
            )
        ...
        if default_namespace is None:
            _namespaces = dict(
                ...
                for _, node in SafeElementTree.iterparse(
                    StringIO(SafeElementTree.tostring(data, "unicode")),
                    events=["start-ns"],
                )
            )
            default_namespace = (
                re_compile(r"^\{(.*?)\}.").search(data.tag)
                or (None, _namespaces.get(""))
            )[1]
        ...
        if data.text:
            for p, pi in klass_properties.items():
                if pi.custom_names.get(SerializationType.XML) == ".":
                    _data[p] = _xs_string_mod_apply(
                        data.text.strip(), pi.xml_string_config
                    )

        # Handle Sub-Elements
        for child_e in data:
            decoded_k = CurrentFormatter.formatter.decode(
                strip_default_namespace(child_e.tag)
            )
            ...
            try:
                ...
                if child_e.text:
                    child_e.text = _xs_string_mod_apply(
                        child_e.text, prop_info.xml_string_config
                    )
                ...
                if prop_info.is_array and prop_info.xml_array_config:
                    ...
                    if array_type == XmlArraySerializationType.NESTED:
                        for sub_child_e in child_e:
                            ...
                            if (
                                not prop_info.is_primitive_type()
                                and not prop_info.is_enum
                            ):
                                ...
                                _data[decoded_k].append(
                                    prop_info.concrete_type(sub_child_e.text)
                                )
                    else:
                        if not prop_info.is_primitive_type() and not prop_info.is_enum:
                            _data[decoded_k].append(
                            ...
                            if prop_info.is_helper_type():
                                _data[decoded_k] = (
                                    prop_info.custom_type.xml_denormalize(
                                        child_e,
                            ...
                            _data[decoded_k].append(
                                prop_info.concrete_type(child_e.text)
                            )
                elif prop_info.custom_type:
                    if prop_info.is_helper_type():
                    ...
                    if prop_info.concrete_type == bool:
                        _data[decoded_k] = (
                            str(child_e.text) in _XML_BOOL_REPRESENTATIONS_TRUE
                        )
                    else:
                        _data[decoded_k] = prop_info.concrete_type(child_e.text)
            except AttributeError as e:
...
class ObjectMetadataLibrary:
    ...
    klass_property_mappings: Dict[
        str, Dict[str, "ObjectMetadataLibrary.SerializableProperty"]
    ] = {}
    ...
    class SerializableClass:
        ...
        def __init__(
            ...
            *,
            ...
            self._ignore_during_deserialization = set(
                ignore_during_deserialization or ()
            )
                ...
                if match:
                    ...
                    if results.get("array_type") in self._SORTED_CONTAINERS_TYPES:
                        ...
                        try:
                            ...
                            self._type_ = eval(
                                f'{mapped_array_type}[{results.get("array_of")}]'
                            )
                            ...
                            for (
                                _k_name,
                                _oml_sc,
                            ) in ObjectMetadataLibrary.klass_mappings.items():
                            ...
                            if _k is None:
                                ...
                                for (
                                    _enum_klass
                                ) in ObjectMetadataLibrary.custom_enum_klasses:
                        ...
                        results.get("array_type", "").replace("typing.", "")
                    ...
                    raise ValueError(
                        f"Unable to handle Property with declared type: {type_}"
                    )
            else:
                # Handle real types
    ...
    def register_klass(
        ...
        _logger.debug(
            "Registering Class %s with custom name %s",
        ...
        for _p in ObjectMetadataLibrary._deferred_property_type_parsing.get(
            klass.__qualname__, ()
        ):
    ...
    @classmethod
    def register_custom_json_property_name(
        cls, qual_name: str, json_property_name: str
    ) -> None:
        prop = cls._klass_property_names.get(qual_name)
        if prop is None:
            cls._klass_property_names[qual_name] = {
                SerializationType.JSON: json_property_name
            }
        else:
    ...
    @classmethod
    def register_custom_xml_property_name(
        cls, qual_name: str, xml_property_name: str
    ) -> None:
        prop = cls._klass_property_names.get(qual_name)
        if prop:
            ...
            cls._klass_property_names[qual_name] = {
                SerializationType.XML: xml_property_name
            }

    @classmethod
    def register_klass_view(cls, klass: Type[_T], view_: Type[ViewType]) -> Type[_T]:
        ObjectMetadataLibrary._klass_views[
...
def type_mapping(type_: type) -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s with custom type: %s",
            f.__module__,
            f.__qualname__,
            type_,
        )
...
def include_none(
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s to include None for view: %s",
            f.__module__,
            f.__qualname__,
            view_,
        )
...
def json_name(name: str) -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s with JSON name: %s", f.__module__, f.__qualname__, name
        )
...
def string_format(format_: str) -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s with String Format: %s",
            f.__module__,
            f.__qualname__,
            format_,
        )
...
def view(view_: Type[ViewType]) -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s with View: %s", f.__module__, f.__qualname__, view_
        )
...
def xml_attribute() -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s as XML attribute", f.__module__, f.__qualname__
        )
...
def xml_string(string_type: XmlStringSerializationType) -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s as XML StringType: %s",
            f.__module__,
            f.__qualname__,
            string_type,
        )
...
def xml_name(name: str) -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s with XML name: %s", f.__module__, f.__qualname__, name
        )
...
def xml_sequence(sequence: int) -> Callable[[_F], _F]:
    ...
    def decorate(f: _F) -> _F:
        _logger.debug(
            "Registering %s.%s with XML sequence: %s",
            f.__module__,
            f.__qualname__,
            sequence,
        )
