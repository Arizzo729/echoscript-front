...
class ComparableDict:
    ...
    def __lt__(self, other: Any) -> bool:
        ...
        return ComparableTuple(self._dict.get(k) for k in keys) < ComparableTuple(
            other._dict.get(k) for k in keys
        )
    ...
    def __new__(cls, purl: "PackageURL") -> "ComparablePackageURL":
        return super().__new__(
            ...
            (
                ...
                purl.version,
                (
                    ComparableDict(purl.qualifiers)
                    if isinstance(purl.qualifiers, dict)
                    else purl.qualifiers
                ),
                purl.subpath,
            ),
        )
