...
class Bom:
    ...
    @serializable.xml_array(
        serializable.XmlArraySerializationType.NESTED, "vulnerability"
    )
    ...
    def get_component_by_purl(
        self, purl: Optional["PackageURL"]
    ) -> Optional[Component]:
        """
    ...
    def _get_all_components(self) -> Generator[Component, None, None]:
        if self.metadata.component:
            yield from self.metadata.component.get_all_nested_components(
                include_self=True
            )
        for c in self.components:
            yield from c.get_all_nested_components(include_self=True)

    def get_vulnerabilities_for_bom_ref(
        self, bom_ref: BomRef
    ...
    def register_dependency(
        ...
        if _d:
            ...
            if depends_on:
                _d.dependencies.update(
                    map(lambda _d: Dependency(ref=_d.bom_ref), depends_on)
                )
        else:
    ...
    def validate(self) -> bool:
        ...
        component_bom_refs = set(
            map(lambda c: c.bom_ref, self._get_all_components())
        ...
        if self.metadata.component and not any(
            map(
