...
from ..serialization import BomRefHelper, LicenseRepositoryHelper
...
class _ComponentScopeSerializationHelper(serializable.helpers.BaseHelper):
    ...
    @classmethod
    def __normalize(
        cls, cs: ComponentScope, view: Type[serializable.ViewType]
    ) -> Optional[str]:
...
class _ComponentTypeSerializationHelper(serializable.helpers.BaseHelper):
    ...
    @classmethod
    def __normalize(
        cls, ct: ComponentType, view: Type[serializable.ViewType]
    ) -> Optional[str]:
...
class Diff:
    ...
    def __lt__(self, other: Any) -> bool:
        if isinstance(other, Diff):
            return _ComparableTuple((self.url, self.text)) < _ComparableTuple(
...
class OmniborId(serializable.helpers.BaseHelper):
    ...
    _VALID_OMNIBOR_ID_REGEX = re.compile(
        r"^gitoid:(blob|tree|commit|tag):sha(1|256):([a-z0-9]+)$"
    )
...
class Swhid(serializable.helpers.BaseHelper):
    ...
    _VALID_SWHID_REGEX = re.compile(
        r"^swh:1:(cnp|rel|rev|dir|cnt):([0-9a-z]{40})(.*)?$"
    )
...
class Component(Dependable):
    ...
    def for_file(absolute_file_path: str, path_for_bom: Optional[str]) -> "Component":
        ...
        if not exists(absolute_file_path):
            raise FileExistsError(
                f"Supplied file path {absolute_file_path!r} does not exist"
            )
    ...
    @serializable.xml_array(
        serializable.XmlArraySerializationType.FLAT, child_name="omniborId"
    )
    ...
    @serializable.view(SchemaVersion1Dot6)
    @serializable.xml_array(
        serializable.XmlArraySerializationType.FLAT, child_name="swhid"
    )
        ...
        if isinstance(other, Component):
            return _ComparableTuple(
                ...
                (
                    ...
                    other.description,
                    other.scope,
                    _ComparableTuple(other.hashes),
                    _ComparableTuple(other.licenses),
                    other.copyright,
                    other.cpe,
                    ...
                    _ComparableTuple(other.properties),
                    _ComparableTuple(other.components),
                    other.evidence,
                    other.release_notes,
                    other.modified,
                    _ComparableTuple(other.authors),
                    _ComparableTuple(other.omnibor_ids),
                    other.manufacturer,
                    _ComparableTuple(other.swhids),
                    other.crypto_properties,
                    _ComparableTuple(other.tags),
                )
            )
        return NotImplemented
